...
3. `prompt.md`：`{{NARRATIVE}}` 恰好出现 1 次；不含 `[[CANONICAL_NARRATIVE_BULLETS]]` / `[[STYLE_BASELINE]]` / source annotation；从 byte 0 起按序唯一保留 `# Role`、`## Workflow`、`### 步骤 1/2/3`、`### 兼容约束`，并要求每个非来源 `block_id` 只在规范 `data-block-id` 精确属性值中临时回显一次，禁止进入 text／tail／其他属性名值。canonical renderer 只允许同包已验证 tokens 选择闭合 `prompt_role`、独立布尔 `composition.strict_brand_rules` 引言变体，并把闭合类型视觉数据静态物化为步骤 2 的七行风格指令；其余 hard shell 必须字节一致，最终模板必须与 tokens 的确定性合成结果精确相等。省略新字段保持默认 prompt bytes；schema 支持不代表 writer 自动推断这些字段，详见[验证契约](../skills/ppt-style-extract/references/style-pack-verification.md)。
