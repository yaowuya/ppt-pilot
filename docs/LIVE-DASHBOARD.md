...
- 十个步骤都展示工作说明和已有成果，已完成与尚未开始的步骤也保留详情；窄屏可横向浏览各步骤，说明不会隐藏。
- 步骤详情包括已记录文档、故事板实际解析页数、审查轮次与未解决阻断数、可预览样张及正式页／待更新／处理中／受阻页数。缺少产物会明确标注；当前步骤同时显示待确认问题或阻断原因。
- 阶段和页面状态来自 run.json、故事板及活跃批次的逐页 transaction；原始 JSON 暂时写入不完整时显示读取提示并自动重试。阶段完成按流程记录显示，文档存在不等于审批或质量检查通过；详情不展开原始文档正文。
