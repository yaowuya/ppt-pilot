...
start 返回实际 URL，如 `http://127.0.0.1:随机端口/`。重复启动会复用当前服务；无需每生成一页重启或刷新。可选 `--port 8765` 固定端口，冲突时明确报错。静态页面需通过该服务打开，直接双击 index.html 无法读取磁盘运行状态。
...
如果宿主不能保留后台进程，可在独立终端用 `serve` 替换 `start` 前台运行，浏览器访问输出地址。使用 Ctrl+C 或 stop 关闭。任务完成后服务保留供查看；不再使用时执行 stop。
