...
4. **主题与风格**：内置仅 `jiawei-product`（嘉为产品，默认）与 `canway-midyear-review`（嘉为年中总结风格）。未指定风格且无已批准选择／工作区偏好时采用嘉为产品，`guided`／`auto` 的确认规则不变；品牌色定制先生成派生风格包。指定已移除的风格 ID 会返回 `style_not_registered`，需要明确改选已注册风格，不会静默替换。
