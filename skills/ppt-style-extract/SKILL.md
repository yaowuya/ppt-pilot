...
- Require `prompt.md` to equal the deterministic composition of the same pack's verified `tokens.json`: only the closed `prompt_role` choice, independent boolean `composition.strict_brand_rules` introduction choice, and seven typed Step-2 directive lines may vary. Other prefix bytes, Step 1, Step 3, compatibility, and output instructions stay byte-exact; omitted options preserve default prompt bytes. See [verification](references/style-pack-verification.md) for allowed values; schema support does not imply that the writer automatically infers these options.
