...
以下风格约定必须在创建风格包时由 `ppt-style-extract` 的 canonical renderer 从该包 `tokens.json` 静态物化。它们是软参考方向，不是逐项锁定令牌；在保持整套演示文稿风格一致性的前提下，布局、层级、卡片组织、信息密度、配色用法与装饰由生成器自主决定。此 authoring seed 中的占位行故意无法通过运行时 verifier，不得复制注册为模板。

AUTHORING_PLACEHOLDER_MUST_BE_REPLACED_BY_CANONICAL_STYLE_DIRECTIVES
