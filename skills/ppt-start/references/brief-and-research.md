...
- **resume（恢复）**：先读取 `run.json`，严格按全局恢复链 `pending_interaction > visual_generation_blocker > visual_generation_transaction > stage scan` 处理；只有前三类 durable control state 均不存在或已完成后，才进入 `stage scan`，再从第一个未完成或脏输入继续，不重复已经批准的工作。
