...
目标最低为 5 不代表任何宿主都能实际运行 5 个任务。**当前固定运行时新批次上限为 5**；纯规划器的 5→10 目标不证明 runtime 会扩大批次。实际并发还受宿主可用容量、剩余页数、活动批次 inventory 和已接受能力约束；不足 5 时明确报告实际数量及受限原因，不能虚报并发，也不能擅自修改宿主配置。DSH 不读取／修改配置或重启宿主来获取容量。非 Git 工作区不降级。

- 通过已接受的宿主适配器、支持 concurrent tasks 与 durable lookup，且容量已知时，按实际容量自动执行目标并发。
- 缺并发或 durable lookup，但仍满足该宿主的完整生成边界时降为 **width 1**；容量未知也保守使用 1，并报告限制。
- 已知可分配容量为 0 时返回 `WAIT`，等待已有任务／宿主容量变化，不记作 `generator_unavailable`，不忙轮询，不取消已有任务腾位。
- 缺少或不符合[宿主隔离适配器](host-isolation-adapters.md)时按原契约 `generator_unavailable` 阻断，禁止当前上下文生成、嵌套 CLI 或凭据探测。这是结构性不可用，不是 `WAIT` 容量状态，不得轮询；验证 prompt-by-value、fresh history、text-only result、attribution 及该宿主的工具边界，不能只相信一个未验证布尔值。Claude／Codex 仍要求 `filesystem=none`、`data_tools=none`；[DSH 普通 subagent](deepseek-harness.md)明确接受继承工具但禁止调用的提示策略，`fresh_isolation`／`native_fresh_isolation` 在该路径只表示 fresh context，不是硬工具隔离。
...
纯规划器输入示例（有 6 个 ready 页面、宿主可分配 10 路时，首次只派出前 5 页；不表示当前固定 runtime 会创建 6 页新批次）：
...
DSH 的容量与任务观测按[普通 subagent 协议](deepseek-harness.md)：`list_agents` 仅用于发现，不轮询、不提供结果或容量保证；`idle`／`ready` 不等于任务终结。只有来源 child ID 匹配的完成证据或对同一 child 的原答案恢复才能释放相应槽位；未知 launch 仍占用预留，不能靠猜测补位。
...
2. 新批次原样使用固定 runtime 的 prepare request，当前 `batch_width` 上限为 5；schema／纯规划器接受 5..10 不授权手工扩大 runtime inventory。选取最多该数量的有序页面；尾批可以只有 1 页，width 不因此写成 1。批内完整 preflight 和已接受宿主能力协商通过后，仍按 pointer-last 持久化。活动批次绝不因升档扩大 inventory 或改写 width；下一批仍受 runtime 实际上限约束。旧 3／4 页批次原位恢复，v1→v2 迁移仍保留原 width 4 的确定性字节。
...
5. generator 按页使用 fresh context，coordinator 按值传入完整冻结 Prompt 并只消费文本。工具与 ambient context 边界按[宿主隔离适配器](host-isolation-adapters.md)处理：Claude／Codex 保持 `filesystem=none`、`data_tools=none`；DSH 使用普通 subagent 的非内容禁工具 wrapper，是提示策略而非硬隔离。每页 validation 可与 sibling 生成重叠，但 candidate／transaction 写入、final promotion、最低 visible blocker 和 run pointer 仍只有 coordinator 按 `ordered_slide_ids` 串行提交。
6. 限流、超时和失败只会降低后续目标，**不授权立即重试**；先执行原有失败归因、退避、修复上限与恢复门禁，再观测是否可调度。某页耗尽修复策略后仍阻断时不能继续新派后续页。

开始生产时提示“自动并发：规划目标 5，当前批次上限 5，实际 X”；目标或限制发生变化时简短更新，例如“规划目标 7，当前批次上限 5，宿主仅可分配 3 路，实际 3”。不向用户提出并发选择，不把规划输出称作已经运行的任务数；实际在途数量以宿主任务及持久归因为准。
