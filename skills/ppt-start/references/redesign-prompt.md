...
1. **自包含性**：`[在此处粘贴你的内容]` 必须被完整内容块替换，文件不得含有未解析占位符、未定义的全大写标识符（如 `INFORMATION_HIERARCHY`）或“见 XX 文档／章节／字段”式外部引用；brief 的信息层级、限定条件、来源与主题令牌必须实际投影进内容块，不得只写名称。
2. **布局语义一致性**：模板步骤 2 的布局组合库是通用实现层；当 brief 的 `layout_family` 不是 Bento 类布局（如状态泳道、时间线）时，内容块必须显式写出 `layout_family` 全称和结构描述，并声明其优先于步骤 2 的枚举，使生成器不会按卡片墙理解泳道页。
3. **枚举与顺序一致**：内容块中的核心标题、论点计数、分组与顺序必须与 brief 锁定内容的结构一致；照抄 assertion_title 导致两者矛盾（如结论顺序与泳道顺序不同）属于编译失败。
