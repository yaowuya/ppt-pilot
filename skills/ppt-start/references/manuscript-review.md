...
如果子 Agent 无法启动、接收者为空、完成事件缺失或结果上下文不匹配，则本轮改用 `inline_fallback`：在同一运行保留 pending 的 cycle／round／五文件 snapshot，只替换本轮模式与对应来源证据；当前上下文在当前步骤中直接读取同一五文件冻结快照和本规范，重新列出主张／来源映射并执行完整审查。`completion_event_missing` 不授权重启 cycle、归零 round 或新建恢复目录。inline 正式审查不得使用视觉产物、先前辩解或用户偏好作为事实支持，报告必须声明“当前上下文降级审查，不具备独立上下文隔离”。
