...
- **层级 Bento**：卡片覆盖约 40%-60%；主卡在面积/字号/明暗上显著强于次卡；主卡可嵌套 KPI 子卡（仅用于同一命题的量化证据）。
- **语义表面**：白色事实卡承载已观察事实；浅蓝证据表面承载证据边界/结论；深色主卡每页至多一张、用作唯一焦点；浅紫表面承载 AI/试点/风险。
- **内容驱动构图**：根据论证/流程/时间/对比/决策语义选型，禁等权卡片墙、禁无语义装饰线。
- **配色**：主品牌蓝 #156BFF 用于最高价值结论/行动/选中态；天空蓝 #65B7F9 次级关系；紫 #8866FD 用于 AI/高风险/回退；深色主卡 #10233F。
- **字体与节奏**：Microsoft YaHei / PingFang SC / Arial / sans-serif；标题 40 / 主命题 30 / 区块标题 24 / 正文 20；外部安全边距 64px、24px 节奏、卡片内边距 24px；圆角 20 / 14、描边 1.2。
- **禁止母题**：左侧长蓝条；等权卡片墙；背景图片/纹理/渐变；穿过文字的连接线；把阶段进行中写成最终验收；用颜色替代必要文字。
