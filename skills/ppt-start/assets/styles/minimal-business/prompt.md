...
- 色彩角色：dominant=#163B52（主强调；用于标题或关键强调）；accent=#E06B45（行动或指标；用于关键数据或行动强调）；support=#D8E7ED（次级表面；用于次级表面或分隔）
- 字体栈：Arial / Microsoft YaHei / PingFang SC / sans-serif
- 字号层级：body=20；body_weight=400；font_stack=["Arial","Microsoft YaHei","PingFang SC","sans-serif"]；micro_label=14；slide_title=40；title_weight=700
- 间距节奏：card_padding=24；outer_margin=64；standard_gap=24
- 形状语言：corner_radius=12；stroke_width=1.5
- 构图规则：card_coverage="20%-40%"；layout_family="content_driven"；primary_secondary_ratio=1.5
- 禁止母题：等权卡片墙；高饱和多色块；背景图片或纹理
