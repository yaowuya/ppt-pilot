<#
.SYNOPSIS
一条命令把当前仓库的 ppt-start Skill 更新到本机两个宿主：
DeepSeek harness（.agents 插件市场）与 Claude Code（用户级技能目录）。

.DESCRIPTION
可选伴随工具。DeepSeek 侧复用 install-deepseek-plugin.ps1 的插件约定；
Claude Code 侧复制到 $HOME\.claude\skills\ppt-start\（旧版备份为 *.bak-<时间戳>，
仅保留最近一份）。-ProjectClaude 额外写入 <RepoRoot>\.claude\skills\。
...
if (-not $SkipDeepSeek) {
    Write-Host '[1/2] DeepSeek harness（插件市场）...'
    $args2 = @{ }
...
} else { Write-Host '[1/2] 跳过 DeepSeek。' }

if (-not $SkipClaudeCode) {
    Write-Host '[2/2] Claude Code（用户级技能）...'
...
} else { Write-Host '[2/2] 跳过 Claude Code。' }
...
Write-Host ''
Write-Host '全部完成。Claude Code 使用 /ppt-start；DeepSeek harness 使用启动词 ppt-start。'
