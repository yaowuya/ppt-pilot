...
Acceptance harness constraint: use deck_id `acceptance-codex-blocker` and write all run artifacts under `ppt-output/acceptance-codex-blocker/`. Work only in the current workspace. Do not merely describe the expected files: execute the installed Skill and persist them. At the end, report the run directory and terminal stage.